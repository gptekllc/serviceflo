CREATE TABLE public.presentation_outputs (
  program_id uuid NOT NULL REFERENCES public.programs(id) ON DELETE CASCADE,
  target text NOT NULL CHECK (target IN ('audience', 'stage')),
...
GRANT SELECT ON public.presentation_outputs TO anon, authenticated;
GRANT INSERT, UPDATE, DELETE ON public.presentation_outputs TO authenticated;
GRANT ALL ON public.presentation_outputs TO service_role;

ALTER TABLE public.presentation_outputs ENABLE ROW LEVEL SECURITY;

CREATE TRIGGER presentation_outputs_updated_at
BEFORE UPDATE ON public.presentation_outputs
FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

CREATE POLICY "presentation_outputs public read"
  ON public.presentation_outputs FOR SELECT
  USING (true);

CREATE POLICY "presentation_outputs coordinator insert"
  ON public.presentation_outputs FOR INSERT
  TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'coordinator'));

CREATE POLICY "presentation_outputs coordinator update"
  ON public.presentation_outputs FOR UPDATE
  TO authenticated
  USING (public.has_role(auth.uid(), 'coordinator'))
  WITH CHECK (public.has_role(auth.uid(), 'coordinator'));

CREATE POLICY "presentation_outputs coordinator delete"
  ON public.presentation_outputs FOR DELETE
  TO authenticated
  USING (public.has_role(auth.uid(), 'coordinator'));

ALTER TABLE public.presentation_outputs REPLICA IDENTITY FULL;
ALTER PUBLICATION supabase_realtime ADD TABLE public.presentation_outputs;
...
BEGIN
  ...
  IF _target IN ('audience', 'both') THEN
    ...
    INSERT INTO public.presentation_outputs (program_id, target, item_id)
    VALUES (_program_id, 'audience', _item_id)
  ...
  IF _target IN ('stage', 'both') THEN
    INSERT INTO public.presentation_outputs (program_id, target, item_id)
    VALUES (_program_id, 'stage', _item_id)
    ON CONFLICT (program_id, target)
    DO UPDATE SET item_id = EXCLUDED.item_id, updated_at = now();
  END IF;
END;
...
BEGIN
  ...
  IF _target IN ('audience', 'both') THEN
    ...
    INSERT INTO public.presentation_outputs (program_id, target, item_id)
    VALUES (_program_id, 'audience', null)
    ON CONFLICT (program_id, target)
    DO UPDATE SET item_id = null, updated_at = now();
  END IF;

  IF _target IN ('stage', 'both') THEN
    INSERT INTO public.presentation_outputs (program_id, target, item_id)
    VALUES (_program_id, 'stage', null)
    ON CONFLICT (program_id, target)
    DO UPDATE SET item_id = null, updated_at = now();
  END IF;
END;
...
BEGIN
  ...
  IF _target = 'audience' THEN
    ...
    INSERT INTO public.presentation_outputs (program_id, target, item_id)
    VALUES (_program_id, 'audience', v_next_id)
    ON CONFLICT (program_id, target)
    DO UPDATE SET item_id = EXCLUDED.item_id, updated_at = now();
  ELSE
    IF v_next_id IS NULL THEN
    ...
    INSERT INTO public.presentation_outputs (program_id, target, item_id)
    VALUES (_program_id, 'stage', v_next_id)
    ON CONFLICT (program_id, target)
    DO UPDATE SET item_id = EXCLUDED.item_id, updated_at = now();
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.set_presentation_item(uuid, text, uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.clear_presentation_target(uuid, text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.advance_presentation(uuid, text, text) FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.set_presentation_item(uuid, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.clear_presentation_target(uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.advance_presentation(uuid, text, text) TO authenticated;
